...
// ─── PVP HELPERS (UPDATED WITH POWER SCALING) ──────────────────

async function evaluatePvpAttack(attackerChar, move) {
    const refereePrompt =
        `You are the referee of an epic 1v1 battle.
        Character: "${attackerChar}" | Technique: "${move}"

        Does this character canonically possess or have the ability to perform this technique?
        - "VALID_MOVE" if it's a known ability or a logical extension of their powers.
        - "INVALID_MOVE" if it's completely made up or belongs to someone else.

        Respond strictly with "INVALID_MOVE" or "VALID_MOVE".`;
    const decision = await queryLLM(refereePrompt, 0.1);
    return decision ? decision.trim().toUpperCase() : "INVALID_MOVE";
}

// Enforces power scaling for block/dodge effectiveness
async function evaluatePvpClash(attackerChar, defenderChar, attackMove, defenseMove) {
    const prompt =
        `You are a strict combat referee evaluating a 1v1 battle based on CANONICAL POWER SCALING and abilities.

        Attacker: "${attackerChar}" | Attack used: "${attackMove}"
        Defender: "${defenderChar}" | Defense used: "${defenseMove}"

        FIRST, judge if the defense is logically capable of mitigating or countering the attack based on:
        - The relative strength, speed, durability, and hax of the characters.
        - The nature of the attack (e.g., physical blast, psychic, reality-warping).
        - If the defender is vastly weaker and the defense is poor, the attack lands fully (damage > 35).
        - If the defender is equal or stronger and uses a proper defense, they take little to no damage (damage 0–15).
        - If they are evenly matched, damage should be moderate (15–30).

        Write exactly 2 descriptive lines depicting the outcome intensely in the active voice.
        End your response strictly with "DAMAGE: [number]" (0 to 50).`;

    const result = await queryLLM(prompt, 0.7);
    return result ? result.trim() : null;
}

// Enforces scaling for unblocked hits
async function evaluatePvpUnmitigated(attackerChar, defenderChar, attackMove) {
    const prompt =
        `You are a strict combat referee evaluating a 1v1 battle based on CANONICAL POWER SCALING.
        The attacker lands a completely UNBLOCKED hit.

        Attacker: "${attackerChar}" | Attack used: "${attackMove}"
        Defender: "${defenderChar}" | Target is completely undefended!

        Determine how devastating this hit is:
        - If the attacker is vastly stronger (e.g., Goku vs Krillin), damage should be 40–60.
        - If they are equal, damage should be 25–40.
        - If the defender is stronger but caught off guard, damage should be 10–25.
        - If the attack is a planet-buster and defender is a regular human, damage = 60 (instant KO).

        Write exactly 2 descriptive lines depicting the impact intensely.
        End your response strictly with "DAMAGE: [number]" (0 to 60).`;
...
module.exports = [
    ...
    // 19. PVP_LOBBY_ACCEPT – Dummy for interceptor
    {
        name: 'pvp_lobby_accept',
        isPrefixless: false,
        execute: async (sock, msg, args) => {
            await sock.sendMessage(msg.key.remoteJid, { text: "Use `.pvp_join <character>` to join." }, { quoted: msg });
        }
    },

    // 20. PVP_CHOOSE – Dummy for compatibility
    {
        name: 'pvp_choose',
        isPrefixless: false,
        execute: async (sock, msg, args) => {
            await sock.sendMessage(msg.key.remoteJid, { text: "Invalid command." }, { quoted: msg });
        }
    },

    // 21. PVP_FIGHT – Process an attack
    {
        name: 'pvp_fight',
        isPrefixless: false,
        execute: async (sock, msg, args) => {
    ...
    // 22. PVP_DEFEND – Process a defense
    {
        name: 'pvp_defend',
        isPrefixless: false,
        execute: async (sock, msg, args) => {
