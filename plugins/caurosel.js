...
└──────────────────────────────┘\n\n_Swipe through the cards below to explore command categories._ 🔮`;
    ...
    try {
        ...
        const msgProto = generateWAMessageFromContent(jid, {
            interactiveMessage: messageContent.interactiveMessage
        }, { userJid: sock.user.id });

        // WhatsApp's client won't render a carousel/native-flow payload unless
        // the relayed stanza carries this <biz> node telling it what kind of
        // interactive content this is. Baileys doesn't add it for us.
        const bizNode = {
            tag: 'biz',
            attrs: {},
            content: [
        ...
        await sock.relayMessage(jid, msgProto.message, {
            messageId: msgProto.key.id,
            additionalNodes: [bizNode]
        });

    } catch (error) {
        console.error("Carousel Menu Render Error:", error);
        await sock.sendMessage(jid, { text: "❌ Failed to render the domain menu. Please try again shortly." }, { quoted: msg });
    }
}

module.exports = [
    // CAROUSEL CARD MENU (.domain)
    {
        name: 'domain',
        isPrefixless: false,
...
• *${settings.prefix}togcstatus* — Post media directly to current group status.
• *${settings.prefix}togcjid <group_jid>* — Post media to a specific group's status.
