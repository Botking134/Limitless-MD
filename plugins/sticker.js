...
async function handleSp2(sock, msg, args) {
    ...
    if (!query) {
        return await sock.sendMessage(jid, {
            text: `❌ *Usage:* \`${config.prefix}sp2 <search term>\`\n*Example:* \`${config.prefix}sp2 Goku\``
        }, { quoted: msg });
    }

    let gifUrls;
    try {
        gifUrls = await klipySearch(query, { limit: 10 });
    } catch (err) {
    ...
    await sock.sendMessage(jid, {
        text: `📦 *Fetching "${query}"* — converting ${gifUrls.length} GIF(s) to stickers...`
    }, { quoted: msg });
    ...
    for (const url of gifUrls) {
        try {
            ...
            console.error(`⚠️ [SP2] Failed to convert one result for "${query}":`, err.message);
        }
